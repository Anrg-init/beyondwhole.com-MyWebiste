// src/features/shareyourdarksecret/PublicFeedSecrets.jsx
...
export default function PublicFeedSecrets({ forcedFilter }) {
  const [msgs, setMsgs] = useState([]);
  const [filter, setFilter] = useState(forcedFilter || "recent"); // 'recent' | 'yesterday' | 'most'

  useEffect(() => {
    try {
      const q = query(collection(db, "publicSecrets"), orderBy("createdAt", "desc"));
      ...
      console.error("PublicFeedSecrets setup error:", err);
      setMsgs([]);
    }
  }, []);
  ...
  async function giveHeart(id) {
    try {
      const ref = doc(db, "publicSecrets", id);
  ...
  return (
    <div style={{ fontFamily: "Manifold, system-ui, sans-serif", color: "#383B39" }}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-sm" style={{ color: "rgba(56,59,57,0.8)", fontFamily: "Manifold, system-ui" }}>{count} secrets</div>
      </div>
      ...
      {count === 0 ? (
        ...
        >
          <div className="mb-2 font-medium" style={{ color: "#383B39", fontFamily: "Manifold, system-ui" }}>No public secrets in this view.</div>
          <div className="text-xs" style={{ color: "rgba(56,59,57,0.7)", fontFamily: "Manifold, system-ui" }}>Share yours above — it may help someone feel less alone.</div>
        </div>
      ) : filter === "most" ? (
